...
window.CHESS_LOUNGE_APP_V22_MODULE_RUNNING = true;
console.log("Chess Lounge app-v22 module running");
...
function requireLogin(callback) {
  ...
  if (page === "play") {
    setText("status", "App startet. Venter på Firebase Auth...");
  }
  ...
  const watchdog = setTimeout(() => {
    ...
    if (page === "play") {
      setText("status", "Firebase Auth svarer ikke enda. Prøv logg inn på nytt.");
      const debug = document.getElementById("playAuthDebug");
      if (debug) {
        debug.textContent = "Auth-status: ingen respons etter 5 sekunder. Dette er trolig grunnen til at ekte spill-app ikke starter.";
      }
    }
  }, 5000);
  ...
  onAuthStateChanged(auth, async (user) => {
    ...
    if (!user) {
      if (page === "play") {
        setText("status", "Ikke innlogget. Sender deg til innlogging...");
        const debug = document.getElementById("playAuthDebug");
        if (debug) debug.textContent = "Auth-status: ingen bruker innlogget.";
      }
    ...
    if (page === "play") {
      setText("status", "Innlogget. Laster spill...");
      const debug = document.getElementById("playAuthDebug");
      if (debug) debug.textContent = "Auth-status: innlogget som " + (user.email || user.uid);
    }
    ...
    if (page === "play") {
      setText("status", "Auth-feil: " + friendlyError(error));
      const debug = document.getElementById("playAuthDebug");
      if (debug) debug.textContent = "Auth-feil: " + (error?.code || error?.message || String(error));
    } else {
      setStatus("Auth-feil: " + friendlyError(error));
    }
  });
}
...
function pieceBase() {
  return `
    <ellipse class="piece-shadow" cx="50" cy="91.5" rx="34" ry="5.7"></ellipse>
    <path class="piece-fill piece-base" d="M20 84 C24 79 31 77 39 77 H61 C69 77 76 79 80 84 L76 94 H24 Z"></path>
    <path class="piece-rim" d="M27 83 H73"></path>
    <path class="piece-rim soft" d="M30 89 H70"></path>
  `;
}
...
function pieceSvg(type) {
  ...
  const bodies = {
    p: `
      <circle class="piece-fill piece-head" cx="50" cy="25" r="13.3"></circle>
      <path class="piece-highlight" d="M44 18 C48 15 55 15 59 20"></path>
      <path class="piece-fill" d="M38 42 C39 36 61 36 62 42 C63 48 58 53 55 57 C61 62 66 69 68 78 H32 C34 69 39 62 45 57 C42 53 37 48 38 42 Z"></path>
      <path class="piece-rim" d="M39 55 H61"></path>
      <path class="piece-rim" d="M35 76 H65"></path>
      ${base}
    `,

    r: `
      <path class="piece-fill" d="M26 17 H38 V27 H44 V17 H56 V27 H62 V17 H74 V39 H26 Z"></path>
      <path class="piece-highlight" d="M31 21 H36 M47 21 H53 M64 21 H69"></path>
      <path class="piece-rim" d="M29 39 H71"></path>
      <path class="piece-fill" d="M36 41 H64 L69 78 H31 Z"></path>
      <path class="piece-highlight" d="M41 47 C47 44 55 44 61 47"></path>
      <path class="piece-rim" d="M36 66 H64"></path>
      <path class="piece-rim" d="M33 78 H67"></path>
      ${base}
    `,

    n: `
      <path class="piece-fill" d="M29 78 C32 63 36 52 44 44 C38 36 39 26 48 15 C52 23 62 25 69 32 C78 41 76 54 67 63 C62 68 55 71 49 74 L67 78 Z"></path>
      <path class="piece-detail mane" d="M51 18 C55 25 57 32 56 39"></path>
      <path class="piece-detail mane" d="M59 27 C63 35 64 43 62 51"></path>
      <path class="piece-highlight" d="M44 44 C51 49 59 50 66 47"></path>
      <path class="piece-detail" d="M38 61 C47 68 56 70 65 68"></path>
      <circle class="piece-dot eye" cx="56" cy="34" r="2.35"></circle>
      <path class="piece-detail" d="M61 42 C65 42 67 44 68 47"></path>
      <path class="piece-rim" d="M32 78 H69"></path>
      ${base}
    `,

    b: `
      <circle class="piece-fill" cx="50" cy="15" r="4.1"></circle>
      <path class="piece-fill" d="M50 16 C64 25 70 39 62 51 C59 56 55 59 51 61 C62 65 67 71 68 78 H32 C33 71 38 65 49 61 C45 59 41 56 38 51 C30 39 36 25 50 16 Z"></path>
      <path class="piece-highlight" d="M43 25 C47 20 54 20 58 25"></path>
      <path class="piece-detail bishop-cut" d="M57 29 L43 52"></path>
      <path class="piece-rim" d="M36 77 H64"></path>
      ${base}
    `,

    q: `
      <circle class="piece-fill" cx="27" cy="25" r="5.5"></circle>
      <circle class="piece-fill" cx="38" cy="17" r="5.7"></circle>
      <circle class="piece-fill" cx="50" cy="13" r="6.1"></circle>
      <circle class="piece-fill" cx="62" cy="17" r="5.7"></circle>
      <circle class="piece-fill" cx="73" cy="25" r="5.5"></circle>
      <path class="piece-fill" d="M24 34 L36 42 L43 28 L50 46 L57 28 L64 42 L76 34 L68 59 H32 Z"></path>
      <path class="piece-highlight" d="M34 45 C43 50 57 50 66 45"></path>
      <path class="piece-rim" d="M32 59 H68"></path>
      <path class="piece-fill" d="M37 61 H63 C67 66 69 72 68 78 H32 C31 72 33 66 37 61 Z"></path>
      <path class="piece-rim" d="M35 77 H65"></path>
      ${base}
    `,

    k: `
      <path class="piece-detail cross" d="M50 7 V28"></path>
      <path class="piece-detail cross" d="M40 17 H60"></path>
      <path class="piece-fill" d="M38 33 C38 22 62 22 62 33 C62 41 56 46 50 49 C44 46 38 41 38 33 Z"></path>
      <path class="piece-highlight" d="M43 31 C47 27 54 27 58 31"></path>
      <path class="piece-fill" d="M34 50 H66 C72 59 70 70 64 78 H36 C30 70 28 59 34 50 Z"></path>
      <path class="piece-rim" d="M37 62 H63"></path>
      <path class="piece-rim" d="M36 77 H64"></path>
      ${base}
    `
  };

  return `<svg class="piece-svg image-staunton-svg" viewBox="0 0 100 100" aria-hidden="true" focusable="false" xmlns="http://www.w3.org/2000/svg">${bodies[type] || ""}</svg>`;
}
...
function initPlayPage(user) {
  ...
  let isApplyingElo = false;
  let unsubscribeGame = null;

  // Må ligge FØR setupSoundButton(), ellers krasjer appen:
  // "Cannot access 'soundEnabled' before initialization"
  ...
  async function finishOnTime(colorFlagged) {
    ...
    try {
      ...
      const winnerUid = colorFlagged === "white" ? activeGame.blackUid : activeGame.whiteUid;
      ...
      playSound("time");
      pulseBoard();
      await applyEloForGame(finalGame);
    } catch (error) {
  ...
  function calculateElo(playerRating, opponentRating, score) {
    ...
    return Math.max(100, Math.round(playerRating + kFactor * (score - expected)));
  }
  ...
  function scoreForUid(game, uid) {
    if (!game || !uid) return 0.5;

    if (game.resultType === "draw") return 0.5;
    if (!game.winnerUid) return 0.5;

    return game.winnerUid === uid ? 1 : 0;
  }

  function buildUpdatedStats(profile, newElo, score) {
    return {
      elo: newElo,
      wins: safeNumber(profile.wins, 0) + (score === 1 ? 1 : 0),
      losses: safeNumber(profile.losses, 0) + (score === 0 ? 1 : 0),
      draws: safeNumber(profile.draws, 0) + (score === 0.5 ? 1 : 0)
    };
  }

  async function applyEloForGame(gameFromCaller, roomCode = currentRoom) {
    if (!roomCode || !gameFromCaller || isApplyingElo) return;
    if (gameFromCaller.status !== "finished") return;
    if (gameFromCaller.eloApplied) return;

    const whiteUid = gameFromCaller.whiteUid || "";
    const blackUid = gameFromCaller.blackUid || "";

    if (!whiteUid || !blackUid) return;
    if (user.uid !== whiteUid && user.uid !== blackUid) return;

    isApplyingElo = true;

    try {
      const lockResult = await runTransaction(ref(db, "games/" + roomCode + "/eloLock"), (current) => {
        if (current) return;
        return user.uid;
      });
      ...
      const latestSnap = await get(ref(db, "games/" + roomCode));
      const latestGame = { ...gameFromCaller, ...(latestSnap.val() || {}) };

      if (latestGame.eloApplied) {
        await update(ref(db, "games/" + roomCode), { eloLock: null });
        return;
      }

      const resultType = latestGame.resultType || (latestGame.winnerUid ? "win" : "draw");
      const finalGame = { ...latestGame, resultType };

      const [whiteSnap, blackSnap] = await Promise.all([
        get(ref(db, "users/" + whiteUid)),
        get(ref(db, "users/" + blackUid))
      ]);

      const whiteProfile = whiteSnap.val() || {};
      const blackProfile = blackSnap.val() || {};

      const whiteOldElo = safeNumber(whiteProfile.elo, 800);
      const blackOldElo = safeNumber(blackProfile.elo, 800);

      const whiteScore = scoreForUid(finalGame, whiteUid);
      const blackScore = scoreForUid(finalGame, blackUid);

      const whiteNewElo = calculateElo(whiteOldElo, blackOldElo, whiteScore);
      const blackNewElo = calculateElo(blackOldElo, whiteOldElo, blackScore);

      const whiteStats = buildUpdatedStats(whiteProfile, whiteNewElo, whiteScore);
      const blackStats = buildUpdatedStats(blackProfile, blackNewElo, blackScore);

      // Først markerer vi hvilket parti ratingen gjelder. Reglene bruker dette.
      await update(ref(db), {
        ["users/" + whiteUid + "/lastRatedGame"]: roomCode,
        ["users/" + blackUid + "/lastRatedGame"]: roomCode
      });

      const now = Date.now();

      await update(ref(db), {
        ["users/" + whiteUid + "/elo"]: whiteStats.elo,
        ["users/" + whiteUid + "/wins"]: whiteStats.wins,
        ["users/" + whiteUid + "/losses"]: whiteStats.losses,
        ["users/" + whiteUid + "/draws"]: whiteStats.draws,
        ["users/" + whiteUid + "/lastRatingChange"]: whiteNewElo - whiteOldElo,
        ["users/" + whiteUid + "/lastRatedAt"]: now,

        ["users/" + blackUid + "/elo"]: blackStats.elo,
        ["users/" + blackUid + "/wins"]: blackStats.wins,
        ["users/" + blackUid + "/losses"]: blackStats.losses,
        ["users/" + blackUid + "/draws"]: blackStats.draws,
        ["users/" + blackUid + "/lastRatingChange"]: blackNewElo - blackOldElo,
        ["users/" + blackUid + "/lastRatedAt"]: now
      });
      ...
      await update(ref(db, "games/" + roomCode), {
        ...
        rating: {
          ...
          whiteNewElo,
          blackNewElo,
          whiteChange: whiteNewElo - whiteOldElo,
          blackChange: blackNewElo - blackOldElo,
          resultType
        }
      });

      const whiteChange = whiteNewElo - whiteOldElo;
      const blackChange = blackNewElo - blackOldElo;

      setText(
        "status",
        `${finalGame.result || "Partiet er ferdig."} Elo oppdatert: Hvit ${signed(whiteChange)}, svart ${signed(blackChange)}.`
      );
    } catch (error) {
      try {
        await update(ref(db, "games/" + roomCode), {
          eloLock: null,
          eloError: friendlyError(error),
          eloErrorAt: Date.now()
        });
      } catch (nestedError) {}

      setText("status", "Partiet er ferdig, men Elo kunne ikke oppdateres: " + friendlyError(error));
    } finally {
      isApplyingElo = false;
    }
  }
  ...
  function updateStatus(game) {
    ...
    if (game?.status === "finished") {
      if (game.rating) {
        const whiteChange = signed(safeNumber(game.rating.whiteChange, 0));
        const blackChange = signed(safeNumber(game.rating.blackChange, 0));
        setText("status", `${game.result || "Partiet er ferdig."} Elo: Hvit ${whiteChange}, svart ${blackChange}.`);
        return;
      }

      if (game.result) {
        setText("status", game.result + " Venter på Elo-oppdatering...");
        return;
      }
    }
  ...
  async function makeMove(from, to) {
    ...
    try {
      ...
      if (chess.in_checkmate()) {
        ...
        winnerUid = move.color === "w" ? game.whiteUid : game.blackUid;
        resultText = `${move.color === "w" ? "Hvit" : "Svart"} vant på sjakk matt.`;
  ...
  function listenToGame(roomCode) {
    ...
    unsubscribeGame = onValue(ref(db, "games/" + roomCode), (snap) => {
      ...
      if (
        ...
        !game.eloApplied &&
        !game.eloLock &&
        (game.whiteUid === user.uid || game.blackUid === user.uid)
      ) {
        applyEloForGame(game, roomCode);
      }
    }, (error) => {
