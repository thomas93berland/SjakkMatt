window.CHESS_LOUNGE_APP_V12_STARTED = true;
...
function requireLogin(callback) {
  ...
  const watchdog = setTimeout(() => {
    ...
    const status = $("status");
    if (page === "play" && status) {
      status.textContent = "Venter fortsatt på innlogging/Firebase. Sjekk at du er logget inn.";
    }
  }, 4500);
  ...
  onAuthStateChanged(auth, async (user) => {
    ...
    if (!user) {
      if (page === "play") {
        setText("status", "Du er ikke innlogget. Sender deg til innlogging...");
      }

      setTimeout(() => go("./index.html"), 650);
      return;
    }
